...
REQUEST_TIMEOUT = float(os.getenv("HR_REQUEST_TIMEOUT", "20"))  # seconds

app = Flask(__name__, template_folder='templates', static_folder='static')
app.config['JSON_AS_ASCII'] = False  # trả tiếng Việt đúng encoding
...
def _pass_through(resp: requests.Response) -> Response:
    """
    Trả nguyên status code + content từ FastAPI.
    Chỉ forward một số header an toàn/quan trọng.
    """
...
@app.route('/')
def index():
    """Trang giao diện chính (templates/index.html)"""
...
# Health check (đồng bộ với endpoint /health của FastAPI)
